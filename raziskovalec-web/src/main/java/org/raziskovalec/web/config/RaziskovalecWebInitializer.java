...
public class RaziskovalecWebInitializer implements WebApplicationInitializer
{
     ...
     * 
     */
    public RaziskovalecWebInitializer()
    {
    ...
    {
        final AnnotationConfigWebApplicationContext rootAppContext = initRootContext(servletContext);
        
        final AnnotationConfigWebApplicationContext servletAppContext = initMvcContext(servletContext, rootAppContext);
        
        initCharacterFilter(servletContext);
        
        initLoggingFilter(servletContext);
        
        servletContext.addListener(new ContextLoaderListener(rootAppContext));
        
        final ServletRegistration.Dynamic servletDynamic = servletContext.addServlet("appServlet",
    ...
    {
        final FilterRegistration.Dynamic filterDynamic = servletContext.addFilter("characterEncoding",
    ...
    {
        final FilterRegistration.Dynamic loggerFilter = servletContext.addFilter("loggerFilter",
                new Slf4jLoggingFilter());
        loggerFilter.addMappingForUrlPatterns(EnumSet.of(DispatcherType.REQUEST), true, "/*");
    }
    
    private AnnotationConfigWebApplicationContext initMvcContext(final ServletContext servletContext,
            final AnnotationConfigWebApplicationContext rootAppContext)
    {
        logger.trace("Init MVC context");
    ...
    private AnnotationConfigWebApplicationContext initRootContext(final ServletContext servletContext)
    {
        logger.trace("Init Root context");
